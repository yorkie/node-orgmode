...
//block
const flagBegin = lexeme(string('#+BEGIN_'));
const flagEnd= lexeme(string('#+END_'));
const blockName = lexeme(regex(/[^\s]+/i));
const content = lexeme(regex(/[^#+END_]*/));
const block = seq(
  startOfLine, flagBegin, blockName, singleLineText, content , flagEnd, singleLineText
).map(
  (val) => {
    let ctt = val[4] ? val[4].split("\n") : 'None';
    let type = "";
    switch (val[2]) {
      case "CENTER":
        type = 'center';
    ...
    return {
      name: val[2],
      type: type,
      parameter: val[3]? val[3] : "None",
      content: ctt,
    };
  }
);
const blocks = block.many();
...
class OrgmodeParser extends IParser {
  ...
  constructor(stream, options) {
    ...
    this._parser = ignore.then(lazy(
      () => seq(blockOptions, blocks, outline.many())
    ));
  ...
  build(tree) {
    ...
    newTree.blocks = tree[1];

    // build outlines
    for (let part of tree[2]) {
